...
- 美术源文件在 `assets/`(文件名带导出时间戳);运行时用 `UE/Graytail/Content/Graytail/` 下的 `.uasset`,代码按 `/Game/Graytail/...` 包路径 LoadObject(对象与包同名)。新增/更新素材跑 `Content/Python/import_png_assets.py` 重导(用法见脚本头注释;**必须完整编辑器跑, 需加 `-unattended`;Claude shell 因无交互桌面无法跑该脚本, 会崩在主框架初始化, 需用户在自己 PowerShell 运行**)。二进制资产走 LFS(.gitattributes 已配)
